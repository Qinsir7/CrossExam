...
The same general flow is callable at `POST /api/v1/reviews`: an unpaid request receives the standard x402 challenge; after payment, CrossExam runs the configured DeepSeek adversarial examiner, validates its structured output, forces every source-, law-, number-, or tool-dependent claim to remain unresolved, and seals the result into an EIP-191-signed `MODEL_ANALYZED` record. This is real paid reasoning, but deliberately not fake factual verification. The separate Transaction Preflight route remains the evidence-backed path for exact supported X Layer trades.
...
`POST /api/v1/reviews/preflight` is the free universal claim map. `POST /api/v1/reviews` is its x402-paid full adversarial counterpart for Legal, Money, Plan, and unrestricted material. It requires an `Idempotency-Key`, calls the configured DeepSeek model server-side, returns the strongest attack, claim verdicts, blind spots and next actions, and signs the exact analysis. It never claims browsing or source verification that did not happen.
