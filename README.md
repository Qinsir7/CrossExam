...
`POST /api/v1/reviews/preflight` is the free universal claim map. `POST /api/v1/reviews` is its x402-paid full adversarial counterpart for Legal, Money, Plan, and unrestricted material. It requires an `Idempotency-Key`, calls the configured DeepSeek model server-side, returns the strongest attack, claim verdicts, blind spots and next actions, and signs the exact analysis. Legal citations can use authority-domain-restricted Tavily search when `CROSSEXAM_TAVILY_API_KEY` is configured; an unavailable search, unmatched official record, or unclear validity signal stays visibly unresolved.
