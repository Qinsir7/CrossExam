...
Evidence (2026-07-21, local and production): the homepage and `/developers` were inspected in the browser at 390, 768, 1280, and 1440 CSS pixels. The 390px homepage showed the entire disabled-until-input `Make it survive` action in the initial viewport, both routes had `scrollWidth === viewport width`, the developer route exposed the correct title and canonical URL, and browser logs contained no application warning or error. The initial production probe caught a missing Vercel SPA fallback that made direct `/developers` requests return 404; the deployed configuration now preserves the `/review-service/*` API proxy first and sends remaining client routes to `index.html`. After release, direct `/developers` returned the app, the production bundle contained the new product and quickstart copy, and a free `PLAN` preflight through `www.cross-exam.xyz/review-service/v1/reviews/preflight` returned a real claim map with the live 0.20 USDT0 paid-review offer and authority search available. The production backend and marketplace contract were not changed. `npm run lint`, all 47 test files / 206 tests, `npm run build`, `npm audit --omit=dev --audit-level=high` (0 vulnerabilities), the tracked environment-file review, and `git diff --check` passed.
