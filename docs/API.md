...
`GET /.well-known/crossexam.json` returns the public capability manifest. It declares the paid assurance endpoints, protected record retrieval, X Layer `eip155:196`, and the `exact` x402 scheme. The Transaction Preflight capability explicitly states its current exact-X-Layer-token-trade boundary, so callers can avoid requesting unsupported paid work. Configure `CROSSEXAM_EXTERNAL_API_URL` to the exact stable public base used by platform callers (currently `https://www.cross-exam.xyz/review-service`). Discovery endpoints and x402 `resource.url` values are both derived from this base, so the endpoint registered with OKX.AI is identical to the resource the client pays for. `CROSSEXAM_PUBLIC_URL` remains the internal reviewer-callback base and is not used as a substitute when the external base is configured.
...
The live paid profile is intentionally limited to **exact X Layer token trades** (`chainId: 196`, `actionType: "TRADE"`) with an explicit `token:xlayer:0x…` `tokenRiskTarget`. Unsupported payments, approvals, deployments, non-X-Layer transactions, cross-chain token labels, and router actions without an identified token receive `422 TRANSACTION_PREFLIGHT_UNSUPPORTED` **before** an x402 payment challenge. CrossExam does not accept payment for a known-unfulfillable evidence scope.
