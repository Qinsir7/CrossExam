...
The live paid profile is intentionally limited to **exact X Layer token trades** (`chainId: 196`, `actionType: "TRADE"`) with an explicit `tokenRiskTarget`. Unsupported payments, approvals, deployments, non-X-Layer transactions, and router actions without an identified token receive `422 TRANSACTION_PREFLIGHT_UNSUPPORTED` **before** an x402 payment challenge. CrossExam does not accept payment for a known-unfulfillable evidence scope.
