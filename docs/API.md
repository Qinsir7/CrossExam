...
`GET /api/v1/review-jobs/{jobId}/ledger` uses the same owner capability and returns the immutable commercial quote (authorization price, capped external budget, minimum viable price, expected gross margin), whether customer authorization has settled, and actual settled external spend grouped by token asset and atomic amount. It deliberately does not convert different assets into a fabricated USD total; gross margin remains explicitly estimated until all revenue and cost legs share an accounting asset. A scope cannot be marked `REQUESTED`, or accept a delivery, until the worker has recorded a successful x402 settlement transaction for it.
