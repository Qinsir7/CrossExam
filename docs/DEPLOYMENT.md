...
- `CROSSEXAM_DEEP_REVIEW_PRICE_USD`: the buyer price floor for the `POST /api/v1/cross-examinations` workflow (default `0.20`). The prepare endpoint is free; a started job must still be authorized through the returned x402 funding capability before any external evidence procurement can occur. Add this variable to Railway only in an authorized additive release.
