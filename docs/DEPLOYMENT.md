...
- `CROSSEXAM_DEEP_REVIEW_PRICE_USD`: the fixed buyer price for `POST /api/v1/reviews` and the buyer price floor for the durable `POST /api/v1/cross-examinations` workflow (default `0.20`). The universal preflight is free; external evidence procurement remains a separate authorized workflow.
