...
- Each active `CROSSEXAM_REVIEWER_REGISTRY` entry needs an HTTPS `procurementEndpoint` and `procurementProtocol: "CROSSEXAM_SIGNED_CALLBACK_V1"` before the worker can send it work. A generic A2MCP/x402 endpoint may supply paid evidence but cannot be registered as a signed reviewer unless it implements the callback contract.
