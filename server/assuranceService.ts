...
export async function aggregateProcurementVerifiedAssurance(
  ...
  await Promise.all(dispatch.assignments.map(async (assignment) => {
    ...
    if (reviewer.procurementProtocol === 'PAID_EVIDENCE_V1'
      ...
      const validPublicTransport = provenance?.kind === 'PUBLIC_API_EVIDENCE_V1'
        && provenance.transport?.scheme === 'PUBLIC_HTTPS' && provenance.transport.marginalCostUsd === 0
      if (!provenance || provenance.sourceId !== reviewer.id
        ...
        || !/^0x[0-9a-f]{64}$/i.test(provenance.responseHash) || (!validPayment && !validAuthentication && !validPublicTransport)) {
